...
public class AdsManager : SingletonClass.Singleton<AdsManager>
{
    private const string MaxSdkKey =
        "GftH7GLUVgPrmMiSI9kkRPJOpg4c3oaq2KLUfec9m25kqRXD_1IesbXhSsFKhyAasSXecfxpI8hXDgpdwvZ6ri";

    private const string InterstitialAdUnitId = "07f8ac5a06985856";
    private const string RewardedAdUnitId = "9073ae48a4c8c08f";
    private const string RewardedInterstitialAdUnitId = "ENTER_REWARD_INTER_AD_UNIT_ID_HERE";
    private const string BannerAdUnitId = "c054ce715cc088cb";
    private const string MRecAdUnitId = "ENTER_MREC_AD_UNIT_ID_HERE";

    [SerializeField] private bool isBannerShowing;
    [SerializeField] private bool isMRecShowing;

    [SerializeField] private int interstitialRetryAttempt;
    [SerializeField] private int rewardedRetryAttempt;
    [SerializeField] private int rewardedInterstitialRetryAttempt;
    ...
    private float _elapsedBeforeADTimer;
    private readonly float _totalTimerBeforeAdTimer = 60f;

    private float _elapsedADTimer;
    private readonly float _totalAdTimer = 75f;

    private void Update()
    {
        if (_elapsedBeforeADTimer < _totalTimerBeforeAdTimer)
        {
            _elapsedBeforeADTimer += Time.deltaTime;
        }
        else
        {
            if (_elapsedADTimer < _totalAdTimer) _elapsedADTimer += Time.deltaTime;
        }
    }
    ...
    public void TryShowInterstitial()
    {
        ...
        if (_elapsedADTimer < _totalAdTimer) return;
    ...
    public void TryShowRewarded(Action rewardAction)
    {
        if (MaxSdk.IsRewardedAdReady(RewardedAdUnitId))
        {
            // rewardedStatusText.text = "Showing";
            MaxSdk.ShowRewardedAd(RewardedAdUnitId);
            OnRewarded = rewardAction;
        }
    }
